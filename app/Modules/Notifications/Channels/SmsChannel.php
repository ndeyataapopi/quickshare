...
class SmsChannel
{
    ...
    protected function formatMessage(string $type, array $data): string
    {
        $templates = [
            ...
            'loan_disbursed' => 'QuickShare: Your loan ' . ($data['reference'] ?? '') . ' has been disbursed. Amount: ' . config('loan.general.currency_symbol', 'N$') . ($data['amount'] ?? '0'),
            'repayment_reminder' => 'QuickShare Reminder: Your repayment of ' . config('loan.general.currency_symbol', 'N$') . ($data['amount'] ?? '0') . ' is due on ' . ($data['due_date'] ?? 'soon') . '.',
            'repayment_overdue' => 'QuickShare URGENT: Your repayment of ' . config('loan.general.currency_symbol', 'N$') . ($data['amount'] ?? '0') . ' is now overdue. Please pay immediately.',
            'repayment_received' => 'QuickShare: Thank you! Your repayment of ' . config('loan.general.currency_symbol', 'N$') . ($data['amount'] ?? '0') . ' has been received.',
